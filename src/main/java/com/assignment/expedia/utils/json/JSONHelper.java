package com.assignment.expedia.utils.json;
...
public class JSONHelper {
	...
	public static String convertObjectToJSONString(Object p_obj) {
		...
		if(p_obj instanceof Collection) {
			return convertArrayToJSON(((Collection) p_obj).toArray()).toString();
		}
    ...
    public static JSONArray convertArrayToJSON(Object[] p_array){
        ...
        for(int l_i = 0;l_i < l_size; l_i++){
            ...
            if(l_value instanceof Map) {
            	...
            	l_object.put(convertArrayToJSON(((Collection) l_value).toArray()).toString());
    ...
    //--------------------------------------------------------------------------
    public static Map<String, Object> convertJSONStringToMap(String p_json_string) {
    	try {
	    	org.json.JSONObject l_obj	=	new org.json.JSONObject(p_json_string);
	    	
	    	return convertJSONToMap(l_obj);
    	} catch (Exception e) {
    		System.out.println("Exception while parsing incoming json string"+e.getMessage());
    		return null;
    	}
    }
    ...
    //--------------------------------------------------------------------------
    public static Map<String, Object> convertJSONToMap(org.json.JSONObject p_json_obj) throws JSONException {
    	...
    	while(l_iter.hasNext()) {
    		...
    		if(l_value instanceof org.json.JSONObject) {
    			l_values.put(l_key, convertJSONToMap((org.json.JSONObject) l_value));
    			continue;
    		}
